...
扩展地址：[VSCode-extension](https://marketplace.visualstudio.com/items?itemName=zhaogl.vscode-extension-sample 'VSCode-extension')

开源地址：[zhaogl/vscode-extension (github.com)](https://github.com/zhaogl/vscode-extension)
...
![](https://img.zhaogl.me/StiMqQrFCi.png)

### 运行 vscode 插件

既然创建好了工程，那必然是要运行的。由于我这里选择的 ts ＋ webpack 进行开发（视情况勾选 webpack），所以是需要打包，同时脚手架已经生成好了对应.vscode 的设置。只需要按下 F5 即可开始调试，这时会打开一个新的 vscode 窗口，`Ctrl+Shift+P`打开命令行，输入`Hello World`，右下角弹出提示框`Hello World from zhaogl-plugin!`

:::warning

注意: 由于是 webpack 开发，在调用堆栈中可以看到有两个进程，一个是 webpack，另一个是新开的插件窗口的，同时在该调试窗口也能查看调试输出信息。

![](https://img.zhaogl.me/Yv4X32qLE5.png)
...
export function activate(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand('zhaogl-plugin.helloWorld', () => {
    vscode.window.showInformationMessage('Hello World from zhaogl-plugin!')
  })
...
`vscode.commands.registerCommand`用于注册命令，`zhaogl-plugin.helloWorld` 为命令 ID，在后续`package.json`中要与之匹配。第二个参数为一个回调函数，当触发该命令时，弹出提示框。

在 package.json 中关注 activationEvents 与 contributes

```json title="package.json"
{
  "activationEvents": ["onCommand:zhaogl-plugin.helloWorld"],
  "contributes": {
    "commands": [
      {
        "command": "zhaogl-plugin.helloWorld",
        "title": "Hello World"
      }
    ]
  }
...
activationEvents 激活事件，`onCommand:zhaogl-plugin.helloWorld`中`zhaogl-plugin`是插件 ID 要与 extension.ts 中的注册命令匹配，`helloWorld`则是命令标识，而 onCommand 则是监听的类型，此外还有`onView`、`onUri`、`onLanguage`等等。

contributes 则是配置那些地方来显示命令，像官方的例子中，就是在 Ctrl + Shift + P 命令行中输入 Hello World 来调用`zhaogl-plugin.helloWorld` 命令。此外还可以设置按键与菜单
...
![](https://img.zhaogl.me/9oqLzZl-wE.png)
...
const commandLine = os.platform() === 'win32' ? `start https://zhaogl.me` : `open https://zhaogl.me`
exec(commandLine)
```

当然了，当用户选择不再提示的时候，下次再打开 vscode 就别提示了，不然大概率就是卸载插件了。这里就需要设置全局参数了，在 package.json 中 contributes 设置 configuration，具体如下，注意`zhaoglPlugin.showTip` 为全局参数之一
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (showTip) {
    ...
    if (result === '是') {
      const commandLine =
        os.platform() === 'win32' ? `start https://zhaogl.me` : `open https://zhaogl.me`
      exec(commandLine)
    } else if (result === '不再提示') {
...
![](https://img.zhaogl.me/3SRybBGaF1.png)
...
export async function activate(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand('zhaogl-plugin.newFile', (uri: vscode.Uri) => {
...
![](https://img.zhaogl.me/nQu3Y8DWSw.png)
...
![](https://img.zhaogl.me/SnnPUABJN5.png)
...
![](https://img.zhaogl.me/yQRsMkT6f5.png)
...
![](https://img.zhaogl.me/-ZCy88xVyq.png)
...
```typescript title="extension.ts"
const commandId = 'zhaogl-plugin.log'
...
这里也一笔带过，具体可看 hover.ts 中的代码。只要在 json 文件中，将鼠标悬停在`zhaogl`这个词中即可触发，试试看看。

![](https://img.zhaogl.me/RUIjdDI90l.png)
...
![](https://img.zhaogl.me/nVO_YmRit4.png)
...
这里在打包前重构下命令 ID，从 zhaogl-plugin → vscode-extension，同时把 package.json 的 name 改成了 vscode-extension-sample，因为发布的时候这个 id 必须唯一，不能与已有重名，到时候生成的为 zhaogl.vscode-extension-sample。（demo 给取了，不然我也不想起名为 sample）
...
![](https://img.zhaogl.me/token1_JNXknLPQyJ.png)

**2、新建一个 token**

![image-20220831152146541](https://img.zhaogl.me/20220831152146541.png)
...
提示 `The Personal Access Token verification succeeded for the publisher 'zhaogl'.` 就说明验证成功
...
运行完毕后，最终提示`Published zhaogl.vscode-extension-sample v1.0.0.` 就说明发布完毕，发布和 npm 包一样，都无需审核，但要求包名唯一。

可以在 [Manage Extensions | Visual Studio Marketplace](https://marketplace.visualstudio.com/manage/publishers/zhaogl 'Manage Extensions | Visual Studio Marketplace') 中管理已发布的插件

![](https://img.zhaogl.me/HssaMdar8f.png)

这时在 vscode 扩展商店中搜索 `vscode-extension-sample`就能找到该插件[VSCode-extension](https://marketplace.visualstudio.com/items?itemName=zhaogl.vscode-extension-sample 'VSCode-extension')，也可以通过`publisher:"zhaogl"`来找到我的所有 vscode 插件。

![vscode-extension](https://img.zhaogl.me/20220711195038039.png)
